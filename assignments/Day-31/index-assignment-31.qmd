...
You likely won't understand these two programs. They are needed to support the rest of the exercises, but are written in a standard dialect of R.
```{webr-r}
#| autorun: true
# generate a random vector with the specified components
rvec <- function(first, ..., rfun=rnorm) {
  dots <- c(first, list(...)) |> unlist()
  if (length(dots) == 1) {
    dots <- rfun(dots[[1]])
